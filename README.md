...
> `llama3.1:8b` is ~4.9GB and `llava:7b` is ~4.7GB. Together they require about **10GB of disk space**.
...
> First model load may take 1–2 minutes. Subsequent calls are faster as Ollama keeps models cached in memory.

---

## Dataset

AXIS does **not require a preloaded dataset or cloud database**.
Investigations are performed directly on the evidence files uploaded by the analyst. All processing happens locally and the uploaded evidence is analyzed in real time by the AI models.
...
* Meta Llama 3.1 — text analysis model
